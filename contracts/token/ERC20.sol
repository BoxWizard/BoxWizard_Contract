...
// 이더리움을 LUCK 토큰과 1:100 비율로 교환해주는 컨트랙트
contract TokenExchange {
    SimpleToken public tokenContract;
    uint256 public exchangeRate;

    event Exchange(address indexed user, uint256 etherAmount, uint256 tokenAmount);

    constructor(address _tokenContract) {
        tokenContract = SimpleToken(_tokenContract);
        exchangeRate = 100; // 1 이더리움 = 100 LUCK
    }
